...
public abstract  class InitOTEvaRunnable<T> extends com.oblivm.backend.network.Client implements Runnable {
	...
	public void runCore() throws Exception {
		FileOutputStream out = new FileOutputStream(new File(Environment.getExternalStorageDirectory(),"otpair.txt"), true);
		FileOutputStream outMatrix = new FileOutputStream(new File(Environment.getExternalStorageDirectory(),"matrix.txt"), true);
