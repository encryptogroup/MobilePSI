...
public class OTExtSender extends OTSender {
	...
	public OTExtSender(int msgBitLength, Network channel, DataInputStream in) {
		super(msgBitLength, channel);
	...
	public void send(GCSignal[][] msgPairs) throws IOException {
		...
		keys = OTExtReceiver.reverseAndExtend(keyPairs, s,
				SecurityParameter.k1, channel, cipher, false);
	}
	...
	private void initialize() throws Exception {
		...
		byte[] buf;
		s = new boolean[SecurityParameter.k1];
		keys = new GCSignal[SecurityParameter.k1];
		for( int j = 0; j < SecurityParameter.k1; ++j ){
			s[j] = input.readBoolean();
			//len = (byte) input.read();
			//System.out.println(len);
			buf = new byte[10];
			input.read(buf);
			//System.out.println(buf);
			keys[j] = new GCSignal(buf);
		}
	}
